 ...
 */
function keywordScore(descriptor, keywords) {
    if (!keywords || keywords.length === 0)
        return 0;
    const haystack = `${descriptor.service} ${descriptor.variant} ${descriptor.title} ${descriptor.description}`.toLowerCase();
    let score = 0;
 ...
 */
function flavorScore(descriptor, flavor) {
    if (!flavor)
        return descriptor.flavor === "system" ? 0.5 : 0.25;
 ...
 */
export function rankPrompts(descriptors, criteria) {
 ...
 */
export function findBestPrompt(descriptors, criteria) {
